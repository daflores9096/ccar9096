<html> 
<head> 
<title>Inventario Pendiente</title> 
...
<tr>
<td width="80%"><font color="#5E8CB5" size="2"><b>INVENTARIO FISICO > Inventario Pendiente</b></font></td>
...
stm_aix("p1i0","p0i0",[0,"Listado para Levantamiento","","",-1,-1,0,"reporte_inventario.php?id_inv=<?php echo $id_inv?>&fecha_lev=<?php echo $fecha_lev?>&descripcion=<?php echo $descripcion?>","_blank","","","","",0,0,0,"","",0,0,0,0,1,"#ffffff",0,"#ffffff",0,"","",3,3,0,0,"#FFFFF7","#000000","#5e8cb5","#F3AC6C","8pt 'Tahoma','Arial'","8pt 'Tahoma','Arial'"]);
stm_ep();
stm_ep();
stm_em();
//-->
...
<td bgcolor="#5E8CB5" width="33%"><font color="ffffff" size="2" face="Courier New, Courier, mono"><b>INVENTARIO PENDIENTE</td>
<td width="33%"><A href="aplicar_inventario.php?id_inv=<?php echo"$id_inv";?>&fecha_lev=<?php echo"$fecha_lev";?>&descripcion=<?php echo"$descripcion";?>" class="linktab">APLICAR INVENTARIO</A></td>
</tr>
</table>
...
<form method="get" name="inventario" action="mod_inventario.php">
<br>
<TABLE border="0" cellpadding="1" cellspacing="2" width="70%"> 
<TR> 
   <TD bgcolor="#FFFFFF"><b><font size="2" color="#5e8cb5">CODIGO:</TD> 
   <td colspan="2"><INPUT TYPE="text" NAME="id_inv" SIZE="10" MAXLENGTH="10" value="<?php echo"$id_inv"; ?>" disabled style="background-color:#FFFFFF "></td>
</TR> 
<TR> 
   <TD bgcolor="#FFFFFF"><b><font size="2" color="#5e8cb5">FECHA LEVANTAMIENTO:</TD> 
   <td colspan="2"><INPUT TYPE="text" NAME="fecha_lev" SIZE="10" MAXLENGTH="10" value="<?php echo"$fecha_lev"; ?>" disabled style="background-color:#FFFFFF "></td>
</TR> 
<TR> 
   <TD bgcolor="#FFFFFF"><b><font size="2" color="#5e8cb5">DESCRIPCION:</TD> 
   <td colspan="2"><INPUT TYPE="text" NAME="descripcion" SIZE="40" MAXLENGTH="40" value="<?php echo"$descripcion"; ?>" disabled style="background-color:#FFFFFF "></td>
</TR> 
...
$cont=0;
   ...
   ";
	...
	$get = mysqli_query($link,"SELECT item.cod_item, item.nom_item, item.unid_item,inventario_aux.existencia_inv 
						FROM item,inventario_aux 
						WHERE inventario_aux.id_inv=$id_inv && item.cod_item=inventario_aux.cod_item
	...
	echo"<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=2 align=center width=100% bgcolor=$arr_color_tabla[2] rules=rows frame=hsides bordercolor=#c1cdd8>
     <tr bgcolor=$arr_color_tabla[0]>";
	   ...
	   echo"<td align=left width=55%><font color=$arr_color_texto[0] size=2><b>&nbsp;ARTICULO</a></font></td>";
	   echo"<td align=left width=15%><font color=$arr_color_texto[0] size=2><b>&nbsp;UNIDAD</a></font></td>";
	   echo"<td align=right width=15%><font color=$arr_color_texto[0] size=2><b>&nbsp;EX. FISICA</a></font></td>";
	 echo"</tr>";
	   $c=0;
	   while($row = mysqli_fetch_array($get)) {
	     echo"<tr bgcolor=$arr_color_tabla[1]>";
         echo"<td align=left><font size=2 color=$arr_color_texto[1]>
         ...
         echo"<td align=right><font size=2 color=$arr_color_texto[1]>
				 <INPUT TYPE=text NAME=cant$c VALUE=$row[existencia_inv]  SIZE=15 MAXLENGTH=15 align=right>
		      </td>";		 
   	     echo"</tr>";
		 $c=$c+1;
	   }
...
<INPUT TYPE=hidden NAME=ide VALUE=$id_inv>
";
...
echo paginacion_orden($total2, $pp, $st, $pag,$orderby,$orden);
echo"</font></center>";  
