	...
	var imgDir = '../tienda/calendario/img/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	var color;
	if (color == null) color = "5e8cb5";
	var imgsrc;
	if (imgsrc == null) imgsrc = new Array('drop_per1.gif','drop_per2.gif','left_per1.gif','left_per2.gif','right_per1.gif','right_per2.gif');
	...
	*/
	...
	var	styleLightBorder = 'border:1px solid #' + color + ';';
	...
	function init() {
		if (!ns4)
		{
			...
			sHTML1 = '<span id="spanLeft" style="border:1px solid #ffffff;cursor:pointer" onmouseover="swapImage(\'changeLeft\',\'left_per2.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollLeftMessage[language]+'\'" onclick="decMonth()" onmouseout="clearInterval(intervalID1);swapImage(\'changeLeft\',\'' + imgsrc[2] + '\');this.style.borderColor=\'#ffffff\';window.status=\'\'" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartDecMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeLeft" src="'+imgDir + imgsrc[2] + '" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanRight" style="border:1px solid #ffffff;cursor:pointer" onmouseover="swapImage(\'changeRight\',\'right_per2.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollRightMessage[language]+'\'" onmouseout="clearInterval(intervalID1);swapImage(\'changeRight\',\'' + imgsrc[4] + '\');this.style.borderColor=\'#ffffff\';window.status=\'\'" onclick="incMonth()" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartIncMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeRight" src="'+imgDir + imgsrc[4] + '" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanMonth" style="border:1px solid #ffffff;cursor:pointer" onmouseover="swapImage(\'changeMonth\',\'drop_per2.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectMonthMessage[language]+'\'" onmouseout="swapImage(\'changeMonth\',\'' + imgsrc[0] + '\');this.style.borderColor=\'#ffffff\';window.status=\'\'" onclick="popUpMonth()"></span>&nbsp;';
			sHTML1 += '<span id="spanYear" style="border:1px solid #ffffff;cursor:pointer" onmouseover="swapImage(\'changeYear\',\'drop_per2.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectYearMessage[language]+'\'" onmouseout="swapImage(\'changeYear\',\'' + imgsrc[0] + '\');this.style.borderColor=\'#ffffff\';window.status=\'\'" onclick="popUpYear()"></span>&nbsp;';
	...
	function closeCalendar() {
		...
		verificar_fecha_reserva();
		/* aca de llama a funciones */
	}
...
//Contruye el listado de los anios
...
//para las posiones donde colocamos los numeros. primeramente obtenemos los espacion en blanco.
		datePointer = 0;
		...
		if (showWeekNumber == 1) {
			sHTML += '<td width="27"><b>' + weekString[language] + '</b></td><td width="1" rowspan="7" bgcolor="#' + color + '" style="padding:0px"><img src="'+imgDir+'divider.gif" width="1"></td>';
		}
		...
		sHTML += '</tr><tr>';
//Primer numero de semana.		
		if (showWeekNumber == 1) {
			sHTML += '<td align="right"><font color="#000066">' + WeekNbr(startDate) + '&nbsp;</font></td>';
		}
//Espacios en blanco antes del primer dia del mes.
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			if ((dayPointer+startAt) % 7 == startAt) {
				sHTML += '</tr><tr>';
//despliga los numeros de semana siguientes.
		...
		document.getElementById('spanMonth').innerHTML = '&nbsp;' +	monthName[language][monthSelected] + '&nbsp;<img id="changeMonth" src="'+imgDir + imgsrc[0] + '" width="12" height="10" border="0">'
	...
	function showCalendar(ctl, ctl2, format, lang, past, color_e, fx, fy) {
